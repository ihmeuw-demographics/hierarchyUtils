---
title: "Introduction to demUtils"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{demUtils}
...
Demographics data is often organized into different groupings of variables. These variables can either be:
...
Iran census population counts included in the package are shown below and documented at `?demUtils::iran_pop`. Examples of categorical variables included in this dataset are `location`, `year`, `sex`, `source_name`, `nid` and `underlying_nid` while `age_start` and `age_end` are one numeric interval variable.
...
The `demUtils` package provides a variety of utility functions that can be used to work with this kind of data. There are functions included to:

* aggregate data to different combinations of the categorical variables or overlapping numeric intervals variables. 

* scale data so that the more detailed values in aggregate equal the aggregate level.

## Aggregate ages and both sexes combined

`demUtils::iran_pop` provides sex and age specific population counts for censuses between `r min(iran_pop$year)` and `r max(iran_pop$year)` in Iran. Each of these censuses includes a different set of most detailed age groups which can make comparisons across censuses more difficult without aggregation to common age groups first.
...
By aggregating to a common set of aggregate age groups using `demUtils::agg` we can more easily compare population counts over time. One of the benefits of this function is that the input data does not need to be square, meaning the age groups in each census year for example can be different.

Here we can first aggregate the interval variable `age` which depends on `age_start` and `age_end` columns in the input dataset. 
...
agg_age_mapping
iran_pop_aggs <- demUtils::agg(dt = iran_pop, 
                               id_cols = id_cols,
...
sex_mapping
iran_pop_aggs_both_sexes <- demUtils::agg(dt = iran_pop_aggs, 
                                          id_cols = id_cols,
                                          value_cols = value_cols,
                                          col_stem = "sex",
...
# this function is used internally by `demUtils::agg` but is available to help
# visualize why some aggregates can't be made, what data is missing, etc.
agg_tree <- demUtils::create_agg_tree(
  mapping = iran_mapping, 
  node_exists = iran_pop_aggs[year == 2006 & source_name != "DYB", location],
  col_type = "categorical")
demUtils::vis_tree(agg_tree)
```

Now we can make location aggregates but need to use two arguments that are helpful when aggregating non-square data. `drop_present_aggs = T` because in some source-years Iran is already included so the function needs to drop the aggregate and recalculate it. `missing_dt_severity = "none"` because in some source-years we don't have the present day provinces and its okay to just make the aggregates that are possible given the data that is available. 

```{r aggregate_location}
iran_pop_aggs_location <- demUtils::agg(dt = iran_pop_aggs, 
                                        id_cols = id_cols,
                                        value_cols = value_cols,
                                        col_stem = "location",
                                        col_type = "categorical",
...
demUtils::gen_name(iran_pop_aggs, col_stem = "age")
